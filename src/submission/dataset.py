...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Ensure length is between 4 and block_size*3/4
        min_length = 4
        max_length = min(len(document), int(self.block_size * 3/4))
        truncate_length = random.randint(min_length, max_length)
        truncated_doc = document[:truncate_length]
        
        # 2. Break into [prefix] [masked_content] [suffix]
        # Make masked_content length random around 1/4 of truncated length
        truncated_len = len(truncated_doc)
        target_mask_len = truncated_len // 4
        
        # Allow variation in mask length from 1 to roughly 1/2 of truncated length
        mask_length = random.randint(1, min(truncated_len - 2, target_mask_len * 2))
        
        # Random starting point for the mask
        # Ensure we leave room for at least 1 character prefix and suffix
        mask_start = random.randint(1, truncated_len - mask_length - 1)
