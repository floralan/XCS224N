...
class DownProjectBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.C = nn.init.xavier_uniform_(torch.empty(1, config.bottleneck_dim, config.n_embd))
    ...
    def forward(self, x_input):
        ...
        ### START CODE HERE
        y = self.attn(x_input, self.ln1(self.C)) + self.C
        y = self.mlp(self.ln2(y)) + y
...
class UpProjectBlock(nn.Module):
    ...
    def forward(self, y, x_input):
        ...
        ### START CODE HERE
        y = self.attn(self.ln1(y), x_input) + x_input
        y = self.mlp(self.ln2(y)) + y
