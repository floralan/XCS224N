...
def finetune(reading_params_path, finetune_corpus_path, pretrain_dataset, block_size, model, finetune_lr=6e-4, writer=None):
    ...
    if reading_params_path:
        ...
        # Configuration for finetuning with pretrained model
        tconf = TrainerConfig(
            max_epochs=10,
            batch_size=256,
        ...
        # Configuration for finetuning without pretrained model
        tconf = TrainerConfig(
            max_epochs=75,
            batch_size=256,
            learning_rate=finetune_lr,
    ...
    # Create dataset from finetune corpus
    with open(finetune_corpus_path, 'r') as f:
        finetune_corpus = f.read()
    
    # Create dataset using the pretrain dataset's vocabulary and special tokens
    fine_tune_dataset = NameDataset(finetune_corpus, pretrain_dataset)
